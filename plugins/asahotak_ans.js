...
module.exports = {
    ...
    async before(m) {
        ...
        if (!m.quoted || !m.quoted.fromMe || !m.quoted.isBaileys || !/Ketik.*asahotak/i.test(m.quoted.text)) return !0
        ...
        if (m.quoted.id == this.asahotak[id][0].id) {
            ...
            if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
                ...
                global.DATABASE._data.users[m.sender].exp += this.asahotak[id][2]
